...
namespace Kinesis::Math {
	...
	Vector3 operator*(const Matrix &m1, const Vector3 &v)
	{
		Vector3 answer = v;
		m1.Transform(answer);
		return answer;
	}
	Vector4 operator*(const Matrix &m1, const Vector4 &v)
	{
		Vector4 answer = v;
		m1.Transform(answer);
		return answer;
	...
	Matrix Matrix::MakeTranslation(const Vector3 &v)
	{
		Matrix t;
		t.setToIdentity();
		t.set(0, 3, v.x());
	...
	Matrix Matrix::MakeScale(const Vector3 &v)
	{
		Matrix s;
		s.set(0, 0, v.x());
		s.set(1, 1, v.y());
	...
	Matrix Matrix::MakeAxisRotation(const Vector3 &v, double theta)
	{
		Matrix r;
		r.setToIdentity();
