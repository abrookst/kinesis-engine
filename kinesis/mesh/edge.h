...
public:
  ...
  Edge(Vertex *vs, Vertex *ve, Face *f) : start_vertex(vs), end_vertex(ve), face(f), next(nullptr), opposite(nullptr) {}
  ...
  Edge* getOpposite() const {
    ...
    return opposite; }
  float Length() const { return (start_vertex->getPos() - end_vertex->getPos()).Magnitude(); };
