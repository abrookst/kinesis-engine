...
Vertex* Mesh::addVertex(const Vector3 &position) {
...
void Mesh::Load(ArgParser *_args) {
  ...
  camera = NULL;
  background_color = Vector3(1,1,1);
  ...
  while (objfile >> token) {
    if (token == "v") {
      ...
      float r,g,b;
      objfile >> r >> g >> b;
      background_color = Vector3(r,g,b);
      ...
      Vector3 reflective,transmissive,emitted;
      objfile >> token >> r >> g >> b;
      assert (token == "reflective");
      reflective = Vector3(r,g,b);
      float roughness = 0;
      objfile >> token;
      if (token == "roughness") {
        objfile >> roughness;
      ...
      objfile >> r >> g >> b;
      emitted = Vector3(r,g,b);
      // materials.push_back(new Material(texture_file,diffuse,reflective,emitted,roughness));
      materials.push_back(new Material(texture_file,diffuse,reflective,transmissive,emitted,roughness,indexOfRefraction));
    } else {
  ...
  if (camera == NULL) {
    ...
    assert (bbox != NULL);
    Vector3 point_of_interest; bbox->getCenter(point_of_interest);
    float max_dim = bbox->maxDim();
    Vector3 camera_position = point_of_interest + Vector3(0,0,4*max_dim);
