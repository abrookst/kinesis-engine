...
{
  bool Triangle::intersect(const Kinesis::Raytracing::Ray &r, Kinesis::Raytracing::Hit &h) const
  {
  ...
  bool Triangle::plane_intersect(const Kinesis::Raytracing::Ray &r, Kinesis::Raytracing::Hit &h) const
  {
  ...
  bool Triangle::triangle_intersect(const Kinesis::Raytracing::Ray &r, Kinesis::Raytracing::Hit &h, Kinesis::Mesh::Vertex *a, Kinesis::Mesh::Vertex *b, Kinesis::Mesh::Vertex *c) const
  {

    // compute the intersection with the plane of the triangle
    Kinesis::Raytracing::Hit h2 = Kinesis::Raytracing::Hit(h);
    if (!plane_intersect(r, h2))
      return 0;
    ...
    double detA = this->det3x3(a->position[0] - b->position[0], a->position[0] - c->position[0], Rd[0],
    ...
    double beta = this->det3x3(a->position[0] - Ro[0], a->position[0] - c->position[0], Rd[0],
                               a->position[1] - Ro[1], a->position[1] - c->position[1], Rd[1],
                               a->position[2] - Ro[2], a->position[2] - c->position[2], Rd[2]) /
                  detA;
    double gamma = this->det3x3(a->position[0] - b->position[0], a->position[0] - Ro[0], Rd[0],
                                a->position[1] - b->position[1], a->position[1] - Ro[1], Rd[1],
                                a->position[2] - b->position[2], a->position[2] - Ro[2], Rd[2]) /
                   detA;
  ...
  double Triangle::det3x3(double a1, double a2, double a3, double b1, double b2, double b3, double c1, double c2, double c3) const
  {
    return a1 * det2x2(b2, b3, c2, c3) - b1 * det2x2(a2, a3, c2, c3) + c1 * det2x2(a2, a3, b2, b3);
  }

  double Triangle::det2x2(double a, double b, double c, double d) const
  {
    return a * d - b * c;
  }
  ...
  {
    glm::vec3 a = (*this)[0]->position;
    glm::vec3 b = (*this)[1]->position;
    glm::vec3 c = (*this)[2]->position;
    glm::vec3 ab = b - a;
    glm::vec3 ac = c - a;
